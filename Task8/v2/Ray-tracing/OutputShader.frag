...
vec3 getSky(vec3 rd) {
	vec3 col = day > 0.5f ? vec3(0.01, 0.01, 0.01) : vec3(0.1, 0.2, 0.7);
...
vec4 castRay(inout vec3 ro, inout vec3 rd) {
	...
	if(it.x > 0.0 && it.x < minIt.x) {
		minIt = it;
		n = planeNormal;
		float del = it.x / MAX_DIST;
		vec3 ids = ro + rd * it.x;
		vec3 sd = vec3(del + mod(floor(ids.y) + floor(ids.x), 2));
		col = vec4(sd, 0.8);
	}
...
{
	//return (2.0 * (gl_FragColor.xy + offset) - u_resolution.xy) / u_resolution.y;
	return ((gl_TexCoord[0].xy + offset) - 0.5) * u_resolution / u_resolution.y;
}
...
vec3 renderAAx4()
{
	vec4 e = vec4(0.000125,-0.000125,0.000375,-0.000375);
